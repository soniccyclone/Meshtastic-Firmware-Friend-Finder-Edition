# friends-persistence Specification

## Purpose

Persist the FriendFinder paired-friends table across power cycles on both ESP32 and nRF52 (T114) builds, so users do not have to re-pair every device every session. Replaces the prior ESP32-only NVS-Preferences path with a unified LittleFS-backed file at `/prefs/friends.proto` that survives reboot, battery swap, and power loss within the bounds of the underlying filesystem's atomicity guarantees. Composes with the brick-fix P0/P1 write-policy gate when that gate lands inside the `SafeFile` / `NodeDB::saveProto` wrapper. Tracked under issue [#25](https://github.com/soniccyclone/Meshtastic-Firmware-Friend-Finder-Edition/issues/25); landed via PR [#28](https://github.com/soniccyclone/Meshtastic-Firmware-Friend-Finder-Edition/pull/28); originating change [`persist-friends-list`](../../changes/archive/2026-05-09-persist-friends-list/).
## Requirements
